...
let xyzdurlArr = [], xyzdhdArr = [],xyzdbodyArr = [],xyzdcount = ''
...
if ($.isNode()) {
  if (process.env.XYZD_URL && process.env.XYZD_URL.indexOf('\n') > -1) {
   xyzdurlArr = process.env.XYZD_URL.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   xyzdurlArr = process.env.XYZD_URL.split()
  };
  if (process.env.XYZD_HD && process.env.XYZD_HD.indexOf('\n') > -1) {
   xyzdhdArr = process.env.XYZD_HD.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   xyzdhdArr = process.env.XYZD_HD.split()
  };
  if (process.env.XYZD_BD && process.env.XYZD_BD.indexOf('\n') > -1) {
   xyzdbodyArr = process.env.XYZD_BD.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   xyzdbodyArr = process.env.XYZD_BD.split()
  };	
